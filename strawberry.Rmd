...
```{r}
strawb4 %<>% pivot_wider(names_from = `units`, values_from = `Value`)
CWT <- ifelse(strawb4$` MEASURED IN CWT`=="(D)",0,strawb4$` MEASURED IN CWT`)
CWT_LB <- as.numeric(CWT)*100
strawb4$` MEASURED IN LB`[1:54]<- CWT_LB
LB_all <- ifelse(strawb4$` MEASURED IN LB`=="(D)"|strawb4$` MEASURED IN LB`=="(Z)"|
         strawb4$` MEASURED IN LB`=="(NA)",0,strawb4$` MEASURED IN LB`)
strawb4$` MEASURED IN LB` <- as.numeric(LB_all)
strawb4 %>% group_by(State) %>% summarise(LB = sum(` MEASURED IN LB`, na.rm = TRUE))
```
```{r}
total_state_LB <- strawb4 %>% group_by(State) %>% summarise(LB = sum(` MEASURED IN LB`, na.rm = TRUE))
